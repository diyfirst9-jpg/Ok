...
bool vkr_lsfg_prepare(VkrLsfg* lsfg, uint32_t width, uint32_t height, VkFormat format) {
    ...
    lsfg->last_input_width = 0;
    lsfg->last_input_height = 0;
    lsfg->last_input_sequence = 0;
    lsfg->plan_calls = 0;
    lsfg->warm_streak = 0;
    lsfg->warm = false;
...
void vkr_lsfg_process(VkrLsfg* lsfg, VkCommandBuffer cmd, VkImage source, uint32_t width,
    ...
    // The LSFG chain is built at the game's real input extent. Never let a swapchain/display
    // extent leak into the optical-flow history: a mismatched capture invalidates all temporal
    // references and is therefore safely bypassed instead of producing a spatially-wrong frame.
    if (width == 0 || height == 0
        || width != lsfg->built_extent.width || height != lsfg->built_extent.height) {
        if ((lsfg->plan_calls++ % LSFG_TELEMETRY_INTERVAL) == 0) {
            LSFG_LOGW("reject frame capture extent %ux%u; chain is %ux%u",
                      width, height, lsfg->built_extent.width, lsfg->built_extent.height);
        }
        lsfg->last_generations = 0;
        lsfg->generated = false;
        return;
    ...
    lsfg->last_generations =
        std::min<size_t>(generations, VKR_LSFG_MAX_GENERATIONS);
    lsfg->last_input_width = width;
    lsfg->last_input_height = height;
    lsfg->last_input_sequence = count;
...
void vkr_lsfg_generate_into(VkrLsfg* lsfg, VkCommandBuffer cmd, uint32_t generation,
    ...
    if (lsfg->last_generations == 0 || generation >= lsfg->last_generations) return;

    // Input/output resolution is intentionally 1:1. The generated image may be composited to
    // the display later, but the LSFG compute target itself must stay at the exact game extent.
    if (width == 0 || height == 0
        || width != lsfg->last_input_width || height != lsfg->last_input_height
        || width != lsfg->built_extent.width || height != lsfg->built_extent.height) {
        LSFG_LOGW("reject generated target extent %ux%u; input=%ux%u chain=%ux%u",
                  width, height, lsfg->last_input_width, lsfg->last_input_height,
                  lsfg->built_extent.width, lsfg->built_extent.height);
        return;
    }
