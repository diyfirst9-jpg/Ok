...
static bool create_render_passes(VkRenderer* r) {
    // Swapchain pass: color attachment, presentation final layout.
    {
        ...
        // The swapchain is rendered in two logical stages when frame generation is active:
        // 1) the real/base scene, then 2) a generated-frame overlay clipped to the selected
        // Game Rect.  Use LOAD here so the second render-pass begin preserves the base scene.
        // The first/base pass performs an explicit vkCmdClearAttachments() below.
...
static bool record_and_submit_frame(VkRenderer* r) {
    ...
    if (has_effects) {
        ...
        // swapchain_pass now LOADs so generated-frame overlays can preserve the base scene.
        // Clear only for the first/base pass; never clear in the generated overlay pass.
        ...
        clear_rect.rect.extent = r->swapchain_extent;
        clear_rect.baseArrayLayer = 0;
        clear_rect.layerCount = 1;
        vkCmdClearAttachments(f->cmd, 1, &clear_attachment, 1, &clear_rect);
    ...
    if (via_composite && game_w) {
        if (r->lsfg && framegen_capacity > 0) {
            ...
            for (uint32_t g = 0; g < gen_count; g++) {
                ...
                // This swapchain image has not been rendered yet. Build the real/base scene
                // first, then overlay ONLY the generated game window on top of it in the SAME
                // render pass. This preserves desktop/Wine chrome and avoids a second pass
                // having to LOAD an image that was never initialized.
                grpbi.clearValueCount = 1;
                grpbi.pClearValues = &clear;
                vkCmdBeginRenderPass(f->cmd, &grpbi, VK_SUBPASS_CONTENTS_INLINE);

                VkClearAttachment gen_clear_attachment = {0};
                gen_clear_attachment.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
                gen_clear_attachment.colorAttachment = 0;
                gen_clear_attachment.clearValue = clear;
                VkClearRect gen_clear_rect = {0};
                gen_clear_rect.rect.offset.x = 0;
                gen_clear_rect.rect.offset.y = 0;
                gen_clear_rect.rect.extent = r->swapchain_extent;
                gen_clear_rect.baseArrayLayer = 0;
                gen_clear_rect.layerCount = 1;
                vkCmdClearAttachments(f->cmd, 1, &gen_clear_attachment, 1, &gen_clear_rect);
