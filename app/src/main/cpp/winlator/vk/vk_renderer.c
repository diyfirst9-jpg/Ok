...
// Picks the window most likely to be "the game": the ready window with the largest on-screen
// area. Scenes with no single dominant window (launcher, alt-tab, a dialog covering most of the
// screen) return -1 so the caller can skip direct-from-Wine frame generation for that frame
// rather than guess and interpolate the wrong thing.
static int32_t find_game_window(const VkScene* s) {
    ...
    for (uint32_t i = 0; i < s->window_count; i++) {
        const VkRenderableWindow* w = &s->windows[i];
        if (!w->texture || !w->texture->ready) continue;
        if (w->texture->width == 0 || w->texture->height == 0) continue;
