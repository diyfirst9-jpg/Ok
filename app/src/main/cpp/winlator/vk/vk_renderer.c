...
// In that case the Java/X11 side may briefly report the frame at its new root position
// while the client surface still carries the old root position.  Vulkan must not render
// those two surfaces apart.  Keep a small previous-scene geometry snapshot and, when a
// large frame moves while an otherwise unchanged child/client is left behind, propagate
// the exact frame delta to that client before the scene is rendered.  This is deliberately
// geometry-only: textures, UVs, FrameGen and the rest of the render pipeline are untouched.
#define VK_REPARENT_CACHE_SLOTS 4

typedef struct VkReparentWindowState {
    ...
    uint32_t height;
    bool valid;
} VkReparentWindowState;

typedef struct VkReparentCache {
    VkRenderer* renderer;
    VkReparentWindowState windows[VK_MAX_RENDERABLE_WINDOWS];
    uint32_t count;
    bool valid;
} VkReparentCache;

static VkReparentCache g_reparent_caches[VK_REPARENT_CACHE_SLOTS];
static pthread_mutex_t g_reparent_cache_mutex = PTHREAD_MUTEX_INITIALIZER;

static bool rect_contains_center(int32_t rx, int32_t ry, uint32_t rw, uint32_t rh,
...
static uint64_t rect_area_u64(uint32_t w, uint32_t h) {
    return (uint64_t)w * (uint64_t)h;
}

static bool same_geometry(const VkRenderableWindow* a, const VkReparentWindowState* b) {
    return a->x == b->x && a->y == b->y
        && a->width == b->width && a->height == b->height;
}

static VkReparentCache* acquire_reparent_cache(VkRenderer* r) {
    VkReparentCache* free_slot = NULL;
    for (uint32_t i = 0; i < VK_REPARENT_CACHE_SLOTS; i++) {
        if (g_reparent_caches[i].renderer == r) return &g_reparent_caches[i];
        if (!g_reparent_caches[i].renderer && !free_slot) free_slot = &g_reparent_caches[i];
    }
    if (free_slot) {
        memset(free_slot, 0, sizeof(*free_slot));
        free_slot->renderer = r;
        return free_slot;
    }
    // Renderer instances are normally one-per-process. If all slots are occupied, use the
    // first slot rather than disabling rendering; it is only a corrective heuristic.
    memset(&g_reparent_caches[0], 0, sizeof(g_reparent_caches[0]));
    g_reparent_caches[0].renderer = r;
    return &g_reparent_caches[0];
}

static void repair_reparented_client_windows(VkRenderer* r, VkScene* s) {
    if (!r || !s || s->window_count == 0) return;

    VkReparentWindowState incoming[VK_MAX_RENDERABLE_WINDOWS];
    memset(incoming, 0, sizeof(incoming));
    uint32_t incoming_count = s->window_count;
    if (incoming_count > VK_MAX_RENDERABLE_WINDOWS) incoming_count = VK_MAX_RENDERABLE_WINDOWS;
    for (uint32_t i = 0; i < incoming_count; i++) {
        const VkRenderableWindow* w = &s->windows[i];
        incoming[i].texture = w->texture;
        incoming[i].x = w->x;
        incoming[i].y = w->y;
        incoming[i].width = w->width;
        incoming[i].height = w->height;
        incoming[i].valid = true;
    }

    pthread_mutex_lock(&g_reparent_cache_mutex);
    VkReparentCache* cache = acquire_reparent_cache(r);

    if (cache->valid) {
        // A pair is considered frame/client only when the old client center was inside the
        // old frame, the frame moved, and the client itself did not move.  Requiring the
        // client to leave the new frame makes the correction conservative and avoids moving
        // unrelated floating windows that happen to be nearby.
        for (uint32_t fi = 0; fi < s->window_count; fi++) {
            ...
            if (fi >= cache->count || !cache->windows[fi].valid) continue;

            const VkReparentWindowState* old_frame = &cache->windows[fi];
            int32_t dx = frame->x - old_frame->x;
            int32_t dy = frame->y - old_frame->y;
            if (dx == 0 && dy == 0) continue;

            // Decoration/frame surfaces are generally larger than their client surface.
            // Avoid treating a tiny cursor/tool window as a parent frame.
            uint64_t frame_area = rect_area_u64(frame->width, frame->height);
            if (frame_area < 64ULL * 64ULL) continue;

            for (uint32_t ci = 0; ci < s->window_count; ci++) {
                if (ci == fi || ci >= cache->count || !cache->windows[ci].valid) continue;
                VkRenderableWindow* client = &s->windows[ci];
                const VkReparentWindowState* old_client = &cache->windows[ci];
                if (!client->texture || !client->width || !client->height) continue;

                // The client must have been stationary according to the incoming scene.
                // This is the key signal for a stale sibling/reparented client.
                if (old_client->texture != client->texture) continue;
                if (old_frame->texture != frame->texture) continue;
                if (!same_geometry(client, old_client)) continue;
                if (!rect_contains_center(old_frame->x, old_frame->y,
                                          old_frame->width, old_frame->height,
                                          old_client->x, old_client->y,
                                          old_client->width, old_client->height)) continue;

                // If the unchanged client is still inside the moved frame, there is no
                // observable frame/client split to repair.
                if (rect_contains_center(frame->x, frame->y, frame->width, frame->height,
                                         client->x, client->y,
                                         client->width, client->height)) continue;

                // Do not propagate a frame's movement into a surface larger than the frame.
                // Also require meaningful overlap with the old frame.
                uint64_t client_area = rect_area_u64(client->width, client->height);
                if (client_area > frame_area) continue;

                client->x += dx;
                client->y += dy;
                VK_LOGI("Reparent client sync: frame[%u] delta=%d,%d -> client[%u] %d,%d",
                        fi, dx, dy, ci, client->x, client->y);
            }
        }
    }

    // Keep the *incoming* geometry, not the corrected geometry, as the baseline.
    // This prevents a stale Wine sibling position from defeating the repair on the next scene. Wine can
    // keep sending the stale sibling position for multiple frames; comparing against the
    // raw baseline lets us apply the same frame delta exactly once per incoming snapshot.
    uint32_t count = incoming_count;
    for (uint32_t i = 0; i < count; i++) {
        cache->windows[i] = incoming[i];
    }
    for (uint32_t i = count; i < VK_MAX_RENDERABLE_WINDOWS; i++) {
        cache->windows[i].valid = false;
    }
    cache->count = count;
    cache->valid = true;
    pthread_mutex_unlock(&g_reparent_cache_mutex);
}
...
static bool record_and_submit_frame(VkRenderer* r) {
    ...
    int32_t game_idx = via_composite ? find_game_window(&snap) : -1;
    ...
    if (via_composite && game_w) {
        if (r->lsfg && framegen_capacity > 0) {
            ...
            for (uint32_t g = 0; g < gen_count; g++) {
                ...
                // "composite เฉพาะ Game Rect -> จอ": replay the ordinary scene draw for this
                // swapchain image, but with the game window's texture swapped for the generated
                // frame. Every other window, the cursor, and desktop chrome get redrawn at their
                // real (non-interpolated) current position/scale -- same xform/scissor/UV math
                ...
                gen_snap.windows[game_idx].texture = &gen_tex;
                                              ...
                                              jobject sceneBuf)
{
    ...
    // Wine may expose a reparented client as a sibling of its visible frame. Repair a stale
    // client root position before this scene becomes the source for normal rendering and LSFG.
