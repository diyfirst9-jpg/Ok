...
static int32_t find_game_window(const VkScene* s) {
    ...
    for (uint32_t i = 0; i < s->window_count; i++) {
        ...
        // Stable tie-break: keep the first candidate at equal score. The caller uses the same
        // selected index for capture and generated-frame compositing in this frame.
        if (score > best_score) {
            best_score = score;
            best = (int32_t)i;
        }
