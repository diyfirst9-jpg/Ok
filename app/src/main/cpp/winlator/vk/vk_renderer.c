...
static bool create_device(VkRenderer* r) {
    ...
    if (has_shader_f16 && vkGetPhysicalDeviceFeatures2) {
    ...
    if (has_shader_f16 && !vkGetPhysicalDeviceFeatures2) {
        VK_LOGW("vkGetPhysicalDeviceFeatures2 unavailable; disabling shaderFloat16");
    }
...
static bool create_render_passes(VkRenderer* r) {
    // Swapchain pass: color attachment, presentation final layout.
    {
        ...
        // The swapchain pass renders the real/base scene and, when frame generation is active,
        // the generated Game-Rect overlay in the same render pass.  CLEAR initializes the
        // acquired swapchain image before the base scene is drawn; the overlay is simply
        // alpha/compositor blended on top and clipped by the Game Rect.
        att.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
        att.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        att.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        att.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        // Acquired swapchain images are already in PRESENT_SRC_KHR from the previous present.
        // Keeping that as the render-pass initial layout also lets the generated overlay begin
        // a second pass and LOAD the real frame without clearing it.
...
static void destroy_one_composite(VkRenderer* r, VkCompositeTarget* c) {
    ...
    if (c->descriptor_set) vkr_free_descriptor_set(r, c->descriptor_set);
    if (c->sampler)        vkDestroySampler(r->device, c->sampler, NULL);
...
static bool create_one_composite(VkRenderer* r, VkCompositeTarget* c, uint32_t w, uint32_t h) {
    ...
    // Sampler + descriptor set: lets a generated frame parked in this target be sampled by the
    // ordinary window-drawing pipeline (see the composite-into-Game-Rect step). Same recipe as
    // create_one_offscreen.
    VkSamplerCreateInfo si = {VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO};
    ...
    si.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    si.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    if (vkCreateSampler(r->device, &si, NULL, &c->sampler) != VK_SUCCESS) return false;

    c->descriptor_set = vkr_alloc_descriptor_set(r);
    if (c->descriptor_set == VK_NULL_HANDLE) return false;

    VkDescriptorImageInfo dii = {0};
    dii.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    dii.imageView = c->view;
    dii.sampler = c->sampler;
    VkWriteDescriptorSet wri = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
    wri.dstSet = c->descriptor_set;
    wri.dstBinding = 0;
    wri.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    wri.descriptorCount = 1;
    wri.pImageInfo = &dii;
...
// NOTE: the old blit_composite_to_swapchain() (full swapchain-sized composite -> swapchain
// blit) was removed here. Base and generated frames now both land in the swapchain via the
// ordinary draw_scene_pass render path (see record_and_submit_frame) instead of a blit, so nothing
// calls it anymore.
#if 0
static void blit_composite_to_swapchain_reference(VkRenderer* r, VkCommandBuffer cmd,
...
#endif // 0 -- reference copy of the removed blit_composite_to_swapchain, kept for diff clarity
...
// Wine can reparent a client X11 window out from under its visible frame/decorations.
// In that case the Java/X11 scene may report the frame at its new root position while the
// client surface is still reported at the old root position.  Vulkan receives only the flat
// renderable-window list, so repair the stale sibling here before the scene is used for both
// normal rendering and FrameGen.
//
// This is deliberately conservative:
//   * compare the current scene against the previous RAW scene;
//   * a frame must have moved;
//   * the candidate client must have been inside the old frame;
//   * the candidate client must itself have not moved;
//   * the client must be outside the new frame, otherwise there is no split to repair;
//   * never move a client larger than the presumed frame.
//
...
// Upscaling is deliberately opt-in.  The scene/effect list is the only authority that can
// enable an upscaler; with no explicit SGSR1 effect the display buffer is kept at the game's
// native backbuffer size.  This avoids silently rendering a native game image into a larger
// swapchain just because the Android SurfaceView is larger.
static bool scene_has_upscaler(const VkScene* s) {
    // SGSR1 is an upscaler only when it is the first effect in the chain.  In any other
    // position it is not given the dedicated low-resolution source/output path.
    return scene_starts_with_sgsr1(s);
}

// Android's SurfaceFlinger can scale a SurfaceView buffer to the window.  Setting the buffer
// geometry lets Vulkan render/present the actual game resolution instead of doing the upscale
// on the GPU.  Do not call this for an explicitly enabled upscaler: that path intentionally
// requests the display/surface resolution as its output target.
static void request_native_buffer_geometry(VkRenderer* r, uint32_t w, uint32_t h,
                                            bool upscale_enabled) {
    if (!r || !r->anw || w == 0 || h == 0) return;

    uint32_t desired_w = w;
    uint32_t desired_h = h;
    if (upscale_enabled) {
        desired_w = r->display_extent.width ? r->display_extent.width : r->surface_extent.width;
        desired_h = r->display_extent.height ? r->display_extent.height : r->surface_extent.height;
        if (is_quarter_turn_transform(r->swapchain_transform)) {
            uint32_t t = desired_w; desired_w = desired_h; desired_h = t;
        }
    }
    if (desired_w == 0 || desired_h == 0) return;

    if (r->native_buffer_geometry_set
        && r->native_buffer_width == desired_w
        && r->native_buffer_height == desired_h) {
        return;
    }

    // Format 0 means keep the native window's current format.  This only changes buffer
    // allocation geometry; it does not resize the Android window itself.
    int rc = ANativeWindow_setBuffersGeometry(r->anw, (int)desired_w, (int)desired_h, 0);
    if (rc == 0) {
        r->native_buffer_geometry_set = true;
        r->native_buffer_width = desired_w;
        r->native_buffer_height = desired_h;
        VK_LOGI("Render resolution policy: %ux%u (%s; no GPU upscale)",
                desired_w, desired_h, upscale_enabled ? "explicit-upscale-output" : "game-native");
    } else {
        VK_LOGW("ANativeWindow_setBuffersGeometry(%ux%u) failed: %d", desired_w, desired_h, rc);
    }
}
...
// desktop compositor because the desktop/background layer is normally a fullscreen texture and
// therefore wins the area comparison before the real game window is even considered.
//
// Selection rules:
//   1. Reject invalid/empty textures.
//   2. Detect fullscreen desktop-like layers (origin near 0,0 and covering almost the whole
//      logical screen). If at least one non-desktop candidate exists, desktop layers are excluded.
//   3. Prefer a rect whose texture aspect/size agrees with its on-screen Game Rect.
//   4. Prefer a texture whose native dimensions agree with source_width/source_height.
//   5. Use visible area only as a tie-breaker, never as the primary identity of the game.
//   6. If the scene really contains only one fullscreen layer, allow it as the game.
//
// This function is the single source of truth for LSFG input selection and generated-frame
...
static int32_t find_game_window(const VkScene* s) {
    ...
    const uint64_t screen_area = (uint64_t)sw * (uint64_t)sh;

    int32_t fallback = -1;
    uint64_t fallback_area = 0;
    bool have_non_desktop = false;

    // First pass: establish whether this scene contains anything other than a desktop layer.
    for (uint32_t i = 0; i < s->window_count; i++) {
        const VkRenderableWindow* w = &s->windows[i];
        if (!w->texture || !w->texture->ready || !w->width || !w->height) continue;
        if (!w->texture->width || !w->texture->height) continue;

        const uint64_t area = (uint64_t)w->width * (uint64_t)w->height;
        if (area > fallback_area) {
            fallback_area = area;
            fallback = (int32_t)i;
        }

        const bool near_origin = (w->x <= 2 && w->y <= 2);
        const bool covers_screen = screen_area > 0 &&
            area >= (screen_area * 90ULL) / 100ULL;
        const bool desktop_like = near_origin && covers_screen;
        if (!desktop_like) have_non_desktop = true;
    }
    ...
    for (uint32_t i = 0; i < s->window_count; i++) {
        ...
        const double rect_w = (double)w->width;
        const double rect_h = (double)w->height;
        const double tex_w = (double)w->texture->width;
        const double tex_h = (double)w->texture->height;
        const double rect_aspect = rect_w / rect_h;
        const double tex_aspect = tex_w / tex_h;

        const bool near_origin = (w->x <= 2 && w->y <= 2);
        const bool covers_screen = screen_area > 0 &&
            (uint64_t)w->width * (uint64_t)w->height >= (screen_area * 90ULL) / 100ULL;
        const bool desktop_like = near_origin && covers_screen;

        // Never choose the desktop/background if a real window is present.
        ...
        const double aspect_error = fabs(rect_aspect - tex_aspect) /
                                    (rect_aspect > tex_aspect ? rect_aspect : tex_aspect);
        if (aspect_error < 0.01) score += 120.0;
        else if (aspect_error < 0.03) score += 90.0;
        else if (aspect_error < 0.08) score += 45.0;
        else score -= 80.0;

        // Prefer native-resolution textures that map cleanly to their Game Rect.
        const double sx = tex_w / rect_w;
        const double sy = tex_h / rect_h;
        const double scale_error = fabs(sx - sy) / (sx > sy ? sx : sy);
        if (scale_error < 0.01) score += 70.0;
        else if (scale_error < 0.05) score += 35.0;
        else score -= 35.0;

        // If the producer exposed source dimensions, matching them is stronger than area.
        if (s->source_width > 0 && s->source_height > 0) {
            const double dw = fabs(tex_w - (double)s->source_width) /
                              (double)s->source_width;
            const double dh = fabs(tex_h - (double)s->source_height) /
                              (double)s->source_height;
            const double source_error = dw > dh ? dw : dh;
            if (source_error < 0.01) score += 180.0;
            else if (source_error < 0.05) score += 100.0;
            else if (source_error < 0.15) score += 25.0;
        }

        // Fullscreen non-desktop candidates are valid (for a genuinely fullscreen game), but
        // they should not automatically dominate a well-matching window.
        const double area_ratio = screen_area > 0
            ? ((double)w->width * (double)w->height) / (double)screen_area : 0.0;
        if (area_ratio >= 0.90 && !desktop_like) score += 20.0;
        else score += area_ratio * 20.0;

        // Stable tie-break: on an exact score tie, prefer the LATER candidate, not the
        // earlier one. Defense in depth for the Java side (VulkanRenderer.updateScene): a
        // parent frame/decoration window is always appended to the scene's window list before
        // its children (collectRenderableWindows visits parent, then recurses into children),
        // so if source_width/source_height ever arrive as 0 here (their strongest signal,
        // +180, becomes unavailable) and aspect/scale scores end up tied between a title-bar
        // frame and its real content child, a strict `>` used to silently hand the frame the
        // win purely because it appears first in the array. `>=` flips that default toward the
        // child instead, so a missing hint degrades gracefully rather than reintroducing the
        // "picked the title bar" bug through the back door.
        if (score >= best_score) {
            best_score = score;
            best = (int32_t)i;
        }
    ...
    // If every candidate was classified as desktop-like, preserve the old safe fallback.
    if (best < 0) best = fallback;

    if (best >= 0) {
        const VkRenderableWindow* w = &s->windows[best];
        VK_LOGI("Game Rect selected: idx=%d rect=%d,%d %ux%u tex=%ux%u score=%.1f%s",
                best, w->x, w->y, w->width, w->height,
                w->texture->width, w->texture->height, best_score,
                (have_non_desktop && w->x <= 2 && w->y <= 2 &&
                 screen_area > 0 &&
                 (uint64_t)w->width * (uint64_t)w->height >= (screen_area * 90ULL) / 100ULL)
                    ? " [fullscreen-game]" : "");
    }
...
static bool record_and_submit_frame(VkRenderer* r) {
    ...
    // Direct-from-Wine framegen source: pick the dominant on-screen window (the game) and feed
    // LSFG its native VkTexture straight from the X11/Wine backing store, instead of the old
    // path of compositing the whole virtual desktop to swapchain resolution first. This means
    // the optical-flow/interpolation shaders run at the game's real render resolution (usually
    // much smaller than the display), which is both the "true" input signal (no desktop chrome,
    // cursor, or other windows bleeding into the flow estimate) and a large win on the scratch
    // buffer bandwidth those shaders push around every frame.
    // Select the game window even when frame generation is disabled: its actual texture size is
    // the native render/present resolution.  An upscaler is opt-in through the effect list.
    int32_t resolution_game_idx = find_game_window(&snap);
    const VkRenderableWindow* resolution_game_w =
        (resolution_game_idx >= 0) ? &snap.windows[resolution_game_idx] : NULL;
    const bool upscale_enabled = scene_has_upscaler(&snap);
    if (r->native_resolution_mode && resolution_game_w && resolution_game_w->texture) {
        request_native_buffer_geometry(r, resolution_game_w->texture->width,
                                       resolution_game_w->texture->height, upscale_enabled);
    }

    int32_t game_idx = via_composite ? resolution_game_idx : -1;
    const VkRenderableWindow* game_w = (game_idx >= 0) ? &snap.windows[game_idx] : NULL;
    if (via_composite && !game_w) {
        // Menu/launcher/multi-window scene with no single dominant window: interpolating any
        // one of them would be a guess, so skip framegen for this frame and fall back to the
        // ordinary single-pass render below (identical to framegen being off).
        via_composite = false;
    }
    uint32_t fg_width  = game_w ? game_w->texture->width  : 0;
    uint32_t fg_height = game_w ? game_w->texture->height : 0;

    // Never silently fall back to a larger GPU render target.  If Android has not rebuilt the
    // swapchain after the buffer-geometry request yet, wait for the surface callback instead of
    // doing an implicit native->display upscale for one frame.  The same rule applies in reverse
    // when an explicitly enabled upscaler switches the output back to display resolution.
    if (r->native_resolution_mode && resolution_game_w) {
        uint32_t expected_w = resolution_game_w->texture->width;
        uint32_t expected_h = resolution_game_w->texture->height;
        if (upscale_enabled) {
            expected_w = r->display_extent.width ? r->display_extent.width : r->surface_extent.width;
            expected_h = r->display_extent.height ? r->display_extent.height : r->surface_extent.height;
            if (is_quarter_turn_transform(r->swapchain_transform)) {
                uint32_t t = expected_w; expected_w = expected_h; expected_h = t;
            }
        }
        if (expected_w > 0 && expected_h > 0
            && (r->swapchain_extent.width != expected_w
                || r->swapchain_extent.height != expected_h)) {
            VK_LOGI("Waiting for %s swapchain: expected=%ux%u actual=%ux%u",
                    upscale_enabled ? "upscaled" : "native", expected_w, expected_h,
    ...
    // r->composite[] is no longer a full-screen "rendered desktop" target: the base/real frame
    // now always renders straight to the swapchain (see final_pass/final_fb below). These
    // buffers are only game-resolution scratch space that LSFG writes generated frames into,
    // which are then sampled back in at composite time — see the generation loop below.
    if (via_composite) {
        ...
        bool composite_stale = !r->composite_built
            ...
            || r->composite[0].width != fg_width
            || r->composite[0].height != fg_height;
        bool chain_stale = r->lsfg
            && vkr_lsfg_needs_rebuild(r->lsfg, fg_width, fg_height, r->swapchain_format);
        if (composite_stale || chain_stale) {
            wait_inflight_frames(r);
            if (!create_composite_targets(r, fg_width, fg_height, composite_needed)) {
                ...
                if (!vkr_lsfg_prepare(r->lsfg, fg_width, fg_height, r->swapchain_format)) {
                    framegen_capacity = 0;
                }
            }
        }
    ...
    // whole desktop into a swapchain-sized composite, then blit" step is gone. Frame generation
    // no longer needs the base frame's output as its source (it samples game_w->texture
    // directly, see below), so there is nothing left for that intermediate buffer to do here.
    VkRenderPass final_pass = r->pipelines.swapchain_pass;
    VkFramebuffer final_fb = r->swapchain_framebuffers[image_index];
    ...
    if (via_composite && game_w) {
        if (r->lsfg && framegen_capacity > 0) {
            // game_w->texture is kept in SHADER_READ_ONLY_OPTIMAL for normal scene sampling
            // (the base-frame draw above just read it in that layout). LSFG's copy step wants
            // VK_IMAGE_LAYOUT_GENERAL, so borrow it for the duration of this call and hand it
            // back in the layout every other reader expects.
            vkr_image_barrier(f->cmd, game_w->texture->image,
                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL,
                              VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                              VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT);

            // *** The actual "capture point" move: LSFG samples the game's own native-resolution
            // Wine/X11 texture directly, not a full-screen composite. ***
            vkr_lsfg_process(r->lsfg, f->cmd, game_w->texture->image,
                             fg_width, fg_height, gen_count);
            ...
            for (uint32_t g = 0; g < gen_count; g++) {
                ...
                // Write the generated (interpolated) frame into a game-resolution scratch
                // buffer -- NOT a swapchain-sized one. This is the bandwidth reduction: the
                // buffer LSFG's compute shaders touch every generated frame is only as big as
                // the game's own backbuffer, not the display.
                vkr_lsfg_generate_into(r->lsfg, f->cmd, g, VK_FRAMES_IN_FLIGHT + g,
                                       gt->image, gt->view, fg_width, fg_height);
                // generate_into leaves it as a storage-image write (GENERAL); make it
                // sampleable for the composite-into-Game-Rect pass below.
                vkr_image_barrier(f->cmd, gt->image,
                                  VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                  VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                  VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                  VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT);

                // Composite ONLY the generated texture into the selected Game Rect.
                // Do not redraw the desktop/background here: the base scene is already present
                // in this swapchain image and must remain untouched outside the Game Rect.
                VkScene gen_snap = snap;
                VkTexture gen_tex = {0};
                gen_tex.image = gt->image;
                gen_tex.view = gt->view;
                gen_tex.sampler = gt->sampler;
                gen_tex.descriptor_set = gt->descriptor_set;
                gen_tex.width = fg_width;
                gen_tex.height = fg_height;
                gen_tex.format = r->swapchain_format;
                gen_tex.layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                gen_tex.ready = true;
                gen_tex.external = true; // borrowed, not owned -- never destroy this

                // Keep exactly one window in the overlay scene. This makes it impossible for a
                // generated frame to overwrite Wine desktop chrome, other windows, or the
                // background even if the source selection contains a fullscreen layer.
                gen_snap.window_count = 1;
                gen_snap.windows[0] = snap.windows[game_idx];
                gen_snap.windows[0].texture = &gen_tex;
                gen_snap.cursor_visible = false;

                // The raster clip uses the SAME Game Rect that selected the LSFG source.
                // Viewport remains the normal full scene viewport; Game Rect is input/overlay
                // clipping only.  This is deliberately not used as the compositor viewport.
                gen_snap.scissor_enabled = true;
                gen_snap.scissor_x = game_w->x;
                gen_snap.scissor_y = game_w->y;
                gen_snap.scissor_w = (int)game_w->width;
                gen_snap.scissor_h = (int)game_w->height;

                VkRenderPassBeginInfo grpbi = {VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO};
                grpbi.renderPass = r->pipelines.swapchain_pass;
                grpbi.framebuffer = r->swapchain_framebuffers[idx];
                grpbi.renderArea.extent = r->swapchain_extent;
                // The render pass CLEAR initializes this newly acquired swapchain image. Build the
                // real/base scene first, then overlay ONLY the generated game window on top
                // in the SAME render pass. This preserves desktop/Wine chrome.
                grpbi.clearValueCount = 1;
                grpbi.pClearValues = &clear;
                vkCmdBeginRenderPass(f->cmd, &grpbi, VK_SUBPASS_CONTENTS_INLINE);

                // First render the complete REAL scene exactly as it would be presented without
                // frame generation. The generated texture is never used for this pass.
                draw_scene_pass(r, f->cmd, &snap, false,
                                r->swapchain_extent.width, r->swapchain_extent.height);

                // Then render one and only one generated window, clipped to the exact Game Rect.
            ...
            if ((r->framegen_real_frames % 120) == 0) {
                ...
                VK_LOGI("framegen delivered real=%llu made=%llu ratio=%.2f planned=%u got=%u "
                        ...
                        draw_ms, gap_ms, fg_width, fg_height);
            }
        }
    }
    ...
    // Base frame and every generated frame now land in their swapchain image via an ordinary
    // graphics render pass (draw_scene_pass into swapchain_pass) -- neither path writes the
    // swapchain image directly from a compute/transfer op anymore, so both just wait at
    // COLOR_ATTACHMENT_OUTPUT like a normal (non-framegen) frame.
...
JNIEXPORT jlong JNICALL JNI_FN(nativeCreate)(JNIEnv* env, jclass clazz,
    ...
    // Native resolution is the default.  Upscaling is never enabled implicitly; an explicit
    // SGSR1 effect in the scene is required before we request a display-resolution buffer.
    r->native_resolution_mode = true;
...
JNIEXPORT void JNICALL JNI_FN(nativeSurfaceCreated)(JNIEnv* env, jclass clazz, jlong handle, jobject surface) {
    ...
    r->native_buffer_geometry_set = false;
    r->native_buffer_width = 0;
    r->native_buffer_height = 0;
...
JNIEXPORT void JNICALL JNI_FN(nativeSurfaceChanged)(JNIEnv* env, jclass clazz, jlong handle, jint w, jint h) {
    ...
    // Keep the actual Android window size separate from the current buffer geometry.  The latter
    // may intentionally be reduced to the game's native render size.  Some Android WSI stacks
    // report the requested buffer dimensions back here, so do not overwrite the remembered
    // display size in that case.
    if (w > 0 && h > 0) {
        const bool is_our_native_buffer = r->native_buffer_geometry_set
            && (uint32_t)w == r->native_buffer_width
            && (uint32_t)h == r->native_buffer_height;
        if (!is_our_native_buffer || r->display_extent.width == 0 || r->display_extent.height == 0) {
            r->display_extent.width = (uint32_t)w;
            r->display_extent.height = (uint32_t)h;
        }
    }
...
JNIEXPORT void JNICALL JNI_FN(nativeSurfaceDestroyed)(JNIEnv* env, jclass clazz, jlong handle) {
    ...
    r->native_buffer_width = 0;
    r->native_buffer_height = 0;
    r->display_extent.width = 0;
    r->display_extent.height = 0;
    pthread_mutex_unlock(&r->render_mutex);
}
