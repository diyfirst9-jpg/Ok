...
static bool record_and_submit_frame(VkRenderer* r) {
    ...
    // Freeze the exact Game Rect used by the captured input frame. Generated pixels describe
    // the same temporal interval as this source frame, so their placement must come from the
    // capture-time geometry rather than any later mutable scene state.
    VkRenderableWindow game_frame = {0};
    if (game_w) game_frame = *game_w;

    uint32_t fg_width  = game_frame.texture ? game_frame.texture->width  : 0;
    uint32_t fg_height = game_frame.texture ? game_frame.texture->height : 0;
    ...
    if (via_composite && game_w) {
        if (r->lsfg && framegen_capacity > 0) {
            ...
            for (uint32_t g = 0; g < gen_count; g++) {
                ...
                // Lock the geometry/UVs to the exact source frame that LSFG captured. This avoids
                // a generated frame being drawn into a newer window position/size after a move,
                // resize, reparent, or transient desktop-layout update.
                gen_snap.windows[game_idx] = game_frame;
                VkTexture gen_tex = {0};
                ...
                // Input/output images are always the same native game resolution. Scaling to the
                // panel happens only through the normal Game Rect transform in draw_scene_pass.
                gen_snap.windows[game_idx].texture = &gen_tex;
                gen_snap.windows[game_idx].u0 = game_frame.u0;
                gen_snap.windows[game_idx].v0 = game_frame.v0;
                gen_snap.windows[game_idx].u1 = game_frame.u1;
                gen_snap.windows[game_idx].v1 = game_frame.v1;
    ...
    // Temporal ordering is strict: the real source frame must reach the display before any
    // generated frames derived from that source. Presenting GEN first caused the visible stream
    // to start at an interpolated position and then jump backward to the real frame.
