...
typedef struct VkCompositeTarget {
    ...
    VkImageView     view;
    VkDeviceMemory  memory;
    // sampler/descriptor_set: added so a generated (interpolated) frame living in one of these
    // targets can be sampled back into the normal window-drawing pipeline (see the
    // composite-into-Game-Rect step in record_and_submit_frame), the same way a VkTexture or
    // VkOffscreen is sampled. Unused for the VK_FRAMES_IN_FLIGHT "current frame" slots.
    VkSampler       sampler;
    VkDescriptorSet descriptor_set;
    VkFramebuffer   framebuffer;
    uint32_t        width, height;
...
typedef struct VkRenderer {
    ...
    // Physical SurfaceView/window extent.  Kept separately because Android may report the
    // swapchain buffer extent after ANativeWindow_setBuffersGeometry() has been changed.
    VkExtent2D       display_extent;

    // Native-resolution presentation policy.  By default the swapchain follows the
    // game's actual backbuffer size; SurfaceFlinger performs any final display scaling.
    // An explicit upscale effect (currently SGSR1) opts back into display-resolution buffers.
    bool             native_resolution_mode;
    bool             native_buffer_geometry_set;
    uint32_t         native_buffer_width;
    uint32_t         native_buffer_height;
