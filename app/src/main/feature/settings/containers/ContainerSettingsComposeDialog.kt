...
class ContainerSettingsComposeDialog @JvmOverloads constructor(
    ...
    private fun createCallbacks(): GameSettingsCallbacks {
        return object : GameSettingsCallbacks {
            ...
            override fun onWindowsVersionChanged(versionIndex: Int) {
                state.selectedWindowsVersion.intValue = versionIndex.coerceIn(
                    0,
                    (state.windowsVersionEntries.value.size - 1).coerceAtLeast(0),
                )
            }
    ...
    private fun loadInitialData() {
        val c = container
        state.windowsVersionEntries.value = WINDOWS_VERSION_PROFILES.map { it.displayName }
        state.selectedWindowsVersion.intValue = readWindowsVersionIndex(c)
    ...
    private fun readWindowsVersionIndex(c: Container?): Int {
        val root = c?.getRootDir() ?: return DEFAULT_WINDOWS_VERSION_INDEX
        val userReg = File(root, ".wine/user.reg")
        if (!userReg.isFile) return DEFAULT_WINDOWS_VERSION_INDEX

        return try {
            WineRegistryEditor(userReg).use { reg ->
                val wineVersion = reg.getStringValue("Software\\Wine", "Version", "")
                    .trim()
                    .lowercase(Locale.ROOT)
                WINDOWS_VERSION_PROFILES.indexOfFirst { it.wineVersionValue == wineVersion }
                    .takeIf { it >= 0 }
                    ?: DEFAULT_WINDOWS_VERSION_INDEX
            }
        } catch (e: Throwable) {
            Log.w(TAG, "Unable to read Windows version", e)
            DEFAULT_WINDOWS_VERSION_INDEX
        }
    }

    private fun saveWindowsVersion(c: Container) {
        val root = c.getRootDir() ?: return
        val userReg = File(root, ".wine/user.reg")
        val systemReg = File(root, ".wine/system.reg")
        val profile = WINDOWS_VERSION_PROFILES.getOrNull(state.selectedWindowsVersion.intValue)
            ?: WINDOWS_VERSION_PROFILES[DEFAULT_WINDOWS_VERSION_INDEX]
            ...
            if (profile != null) {
                ...
                WineRegistryEditor(systemReg).use { reg ->
                    ...
                    if (profile.csdVersion != null) {
                        ...
                        reg.removeValue(ntKey, "CSDVersion")
                    }
                }
            }
        } catch (e: Throwable) {
            Log.e(TAG, "Unable to save Windows version", e)
        }
    }
    ...
    companion object {
        ...
        private val WINDOWS_VERSION_PROFILES = listOf(
        ...
        private const val DEFAULT_WINDOWS_VERSION_INDEX = 6
