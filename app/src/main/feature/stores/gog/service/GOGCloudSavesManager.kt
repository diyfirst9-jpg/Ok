...
class GOGCloudSavesManager(
    ...
    private suspend fun uploadFile(
        ...
        try {
            ...
            if (!localFile.exists() || !localFile.isFile) throw IllegalStateException("Local save missing: ${file.relativePath}")
            val gzipped = gzipWithZeroMtime(localFile.readBytes())
            val etag = MessageDigest.getInstance("MD5").digest(gzipped).joinToString("") { "%02x".format(it) }
            ...
            val requestBuilder = Request.Builder().url(url).put(requestBody)
                ...
                .header("Etag", etag)
            file.updateTime?.let { requestBuilder.header("X-Object-Meta-LocalLastModified", it) }
            httpClient.newCall(requestBuilder.build()).execute().use { response ->
                if (!response.isSuccessful) {
                    Timber.tag("GOG-CloudSaves").e("Failed to upload ${file.relativePath}: HTTP ${response.code}")
                    throw IllegalStateException("GOG upload failed: ${file.relativePath} HTTP ${response.code}")
                }
            }
            true
        } catch (e: Exception) {
    ...
    private suspend fun downloadFile(
        ...
        try {
            val downloaded = downloadObject(userId, clientId, dirname, file, authToken) ?: throw IllegalStateException("GOG download failed: ${file.relativePath}")
            val expectedHash = file.md5Hash.trim().lowercase()
            if (expectedHash.isNotEmpty()) {
                val actualHash = MessageDigest.getInstance("MD5").digest(gzipWithZeroMtime(downloaded.bytes)).joinToString("") { "%02x".format(it) }
                if (!actualHash.equals(expectedHash, ignoreCase = true)) {
                    Timber.tag("GOG-CloudSaves").e("Hash verification failed for ${file.relativePath}: expected=$expectedHash actual=$actualHash")
                    throw IllegalStateException("GOG hash mismatch: ${file.relativePath}")
                }
            }
            val localFile = File(syncDir, file.relativePath)
            localFile.parentFile?.mkdirs()
            val tempFile = File(localFile.parentFile, ".${localFile.name}.gogtmp")
            FileOutputStream(tempFile).use { it.write(downloaded.bytes); it.fd.sync() }
            try {
                Files.move(tempFile.toPath(), localFile.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE)
            } catch (_: Exception) {
    ...
    private suspend fun verifyCloudHashes(
        userId: String, clientId: String, dirname: String, authToken: String, expected: List<SyncFile>
    ): Boolean {
        if (expected.isEmpty()) return true
        val cloud = getCloudFiles(userId, clientId, dirname, authToken).filter { !it.isDeleted }.associateBy { it.relativePath }
        return expected.all { local -> cloud[local.relativePath]?.md5Hash.equals(local.md5Hash.orEmpty(), ignoreCase = true) }
    }
    ...
    private suspend fun deleteCloudFile(
        ...
        try {
            ...
            httpClient.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    throw IllegalStateException("GOG delete failed: ${file.relativePath} HTTP ${response.code}")
                }
            }
            true
        } catch (e: Exception) {
