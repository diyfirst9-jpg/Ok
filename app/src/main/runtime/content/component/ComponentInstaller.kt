 ...
 * (including set_windows/use_windows, applied directly to the registry files).
 * Verbs that require a running Wine process (install_exe/msi, register_dll/regsvr32, CAB extraction,
 * font registration) throw [InstallException] so the UI can report them clearly.
 *
...
class ComponentInstaller(
    ...
    private fun runSteps(steps: List<Map<String, Any?>>) {
        ...
        for (step in steps) {
            ...
            when (val action = step["action"] as? String) {
                ...
                "set_windows", "use_windows" -> setWindowsVersion(step)
    ...
    /**
     * Sets the emulated Windows version for the container (equivalent to `winecfg`'s
     * "Windows version" dropdown / `wine reg add ... Version`). Manifests express this as
     * `set_windows` / `use_windows` with a `version` field such as "win7", "winxp", "win10".
     */
    private fun setWindowsVersion(step: Map<String, Any?>) {
        val raw = (step["version"] as? String)
            ?: (step["windows"] as? String)
            ?: (step["value"] as? String)
            ?: throw InstallException("set_windows step is missing a 'version' value")
        val version = raw.trim().lowercase(Locale.ROOT)
        val profile = WINDOWS_VERSION_PROFILES[version]
            ?: throw InstallException("Unknown Windows version for set_windows: $raw")

        listener.onStatus("Setting Windows version to ${profile.displayName}")
        listener.onProgress(-1f)

        WineRegistryEditor(userReg).use { reg ->
            reg.setCreateKeyIfNotExist(true)
            reg.setStringValue("Software\\Wine", "Version", profile.wineVersionValue)
        }

        WineRegistryEditor(systemReg).use { reg ->
            reg.setCreateKeyIfNotExist(true)
            val ntKey = "Software\\Microsoft\\Windows NT\\CurrentVersion"
            reg.setStringValue(ntKey, "CurrentVersion", profile.currentVersion)
            reg.setStringValue(ntKey, "CurrentBuildNumber", profile.buildNumber)
            reg.setDwordValue(ntKey, "CurrentMajorVersionNumber", profile.majorVersion)
            reg.setDwordValue(ntKey, "CurrentMinorVersionNumber", profile.minorVersion)
            if (profile.csdVersion != null) {
                reg.setStringValue(ntKey, "CSDVersion", profile.csdVersion)
            }
        }
    }
    ...
    /** One row of `winecfg`'s Windows-version table, keyed by the manifest's version string. */
    private data class WindowsVersionProfile(
        val displayName: String,
        val wineVersionValue: String,
        val currentVersion: String,
        val buildNumber: String,
        val majorVersion: Int,
        val minorVersion: Int,
        val csdVersion: String?,
    )
    ...
    companion object {
        ...
        // Mirrors the versions winecfg exposes; accepts common aliases used by manifests.
        private val WINDOWS_VERSION_PROFILES: Map<String, WindowsVersionProfile> = buildMap {
            fun put(profile: WindowsVersionProfile, vararg keys: String) {
                for (k in keys) put(k, profile)
            }
            put(
                WindowsVersionProfile("Windows XP", "winxp", "5.1", "2600", 5, 1, "Service Pack 3"),
                "winxp", "xp", "win_xp",
            )
            put(
                WindowsVersionProfile("Windows 2003", "win2003", "5.2", "3790", 5, 2, "Service Pack 2"),
                "win2003", "win2k3", "winserver2003",
            )
            put(
                WindowsVersionProfile("Windows Vista", "vista", "6.0", "6002", 6, 0, "Service Pack 2"),
                "vista", "winvista",
            )
            put(
                WindowsVersionProfile("Windows 7", "win7", "6.1", "7601", 6, 1, "Service Pack 1"),
                "win7", "windows7",
            )
            put(
                WindowsVersionProfile("Windows 8", "win8", "6.2", "9200", 6, 2, null),
                "win8", "windows8",
            )
            put(
                WindowsVersionProfile("Windows 8.1", "win81", "6.3", "9600", 6, 3, null),
                "win81", "windows81", "win8.1",
            )
            put(
                WindowsVersionProfile("Windows 10", "win10", "10.0", "19045", 10, 0, null),
                "win10", "windows10",
            )
            put(
                WindowsVersionProfile("Windows 11", "win11", "10.0", "22631", 10, 0, null),
                "win11", "windows11",
            )
        }
