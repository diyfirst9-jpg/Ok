...
public class ViewTransformation {
  ...
  /**
   * Native mode: never scale or center the guest image. The guest/X-server rectangle is
   * rendered at exactly its own pixel size from (0,0). Any enlargement is therefore an
   * explicit user-selected effect/output operation, not an implicit renderer decision.
   */
  public static final int FILL_MODE_NATIVE = 3;

  /** Native is the safe default: no implicit upscale/downscale and no forced centering. */
  public int mode = FILL_MODE_NATIVE;
   ...
   */
  public int effectiveMode = FILL_MODE_NATIVE;
  ...
  public void update(int outerWidth, int outerHeight, int innerWidth, int innerHeight) {
    ...
    // Native mode is deliberately immune to fullscreen's old force-stretch behavior.
    // Fullscreen may change the Android window, but it must not change the game's render
    // resolution or silently introduce a scale operation.
    effectiveMode = (mode == FILL_MODE_NATIVE) ? FILL_MODE_NATIVE
        : (forceStretch ? FILL_MODE_STRETCH : mode);

    if (effectiveMode == FILL_MODE_NATIVE) {
      aspect = 1.0f;
      viewOffsetX = 0;
      viewOffsetY = 0;
  ...
  public float[] mapSurfaceToScene(float surfaceX, float surfaceY, float[] out) {
    ...
    if (effectiveMode == FILL_MODE_NATIVE) {
      out[0] = surfaceX;
      out[1] = surfaceY;
    } else if (effectiveMode == FILL_MODE_STRETCH) {
  ...
  public float mapSurfaceXToScene(float x) {
    ...
    float result = effectiveMode == FILL_MODE_NATIVE ? x
        : (effectiveMode == FILL_MODE_STRETCH ? x * sceneScaleX : (x - viewOffsetX) / aspect);
    return Math.max(0f, Math.min(result, viewWidth / aspect - 1f));
  }

  public float mapSurfaceYToScene(float y) {
    if (aspect <= 0f) return y;
    float result = effectiveMode == FILL_MODE_NATIVE ? y
        : (effectiveMode == FILL_MODE_STRETCH ? y * sceneScaleY : (y - viewOffsetY) / aspect);
