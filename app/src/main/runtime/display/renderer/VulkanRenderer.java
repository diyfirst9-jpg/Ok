    ...
    private void buildAndSubmitFrame(long handle) {
        ...
        try (XLock lock = xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.DRAWABLE_MANAGER)) {
            ...
            for (int i = startIndex; i < renderableWindows.size() && winCount < MAX_WINDOWS; i++) {
                ...
                // title-bar/border chrome that Wine draws around the real client area) — it
                // must never win as the frame-gen source in ANY tier, focused or not, because
                // the actual game render always lives in a leaf descendant window instead.
                boolean isContainerFrame = rw.window != null && hasMappedChild(rw.window);
                if (!isContainerFrame && interactedWindow != null && rw.window != null) {
                    if (isDescendantOf(rw.window, interactedWindow)) {
                        ...
                        candidateTier = 2;
                    } else if (rw.window == interactedWindow) {
                        candidateTier = 1;
                    }
                }
                if (!isContainerFrame && candidateW > 0 && candidateH > 0) {
