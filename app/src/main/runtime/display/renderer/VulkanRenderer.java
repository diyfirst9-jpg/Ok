...
public class VulkanRenderer
    ...
    // Live root-space geometry for every X window content.  RenderableWindow keeps a cached
    // position, but Wine can reparent a game's client surface independently from its frame.
    // Resolve the position from the X window tree each render so frame/client never drift.
    private final IdentityHashMap<Drawable, int[]> liveContentPositions = new IdentityHashMap<>();
    ...
    public void onDrawFrame() {
        if (nativeHandle == 0) return;
        // Geometry can change without a separate render request while Wine is dragging or
        // reparenting a client. Refresh the X->root mapping immediately before composing.
        refreshLiveWindowGeometry();
        buildAndSubmitFrame();
    }
    ...
    private void buildAndSubmitFrame() {
        ...
        try (XLock lock = xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.DRAWABLE_MANAGER)) {
            ...
            for (int i = startIndex; i < renderableWindows.size() && winCount < MAX_WINDOWS; i++) {
                ...
                int rootX = rw.rootX;
                int rootY = rw.rootY;
                int[] livePos = liveContentPositions.get(rw.content);
                if (livePos != null) {
                    rootX = livePos[0];
                    rootY = livePos[1];
                }
                buf.putInt(gOff,      rootX);
                buf.putInt(gOff + 4,  rootY);
    ...
    public void onUpdateWindowGeometry(final Window window, boolean resized) {
        // Never update only the moved frame's cached position. Wine may keep the real game
        // client as a reparented/sibling X window, so rebuild the complete root-space mapping
        // for both moves and resizes. The render thread also refreshes it once per frame.
        xServerView.queueEvent(this::updateScene);
        requestRenderCoalesced();
    }
    ...
    private void collectRenderableWindows(Window window, int x, int y) {
        ...
        if (window != xServer.windowManager.rootWindow) {
            ...
            if (viewable) {
                ...
                if (content != null) {
                    // getRootX/Y are authoritative for the actual X window and handle
                    // reparenting better than accumulating parent-relative coordinates.
                    int rootX = window.getRootX();
                    int rootY = window.getRootY();
                    liveContentPositions.put(content, new int[] {rootX, rootY});
                    renderableWindows.add(new RenderableWindow(content, rootX, rootY));
                }
            }
        }
    ...
    // Re-resolve every content position immediately before a frame is submitted. This covers
    // games that move the Wine frame and client in different X transactions, where an event
    // callback can arrive between two render submissions.
    private void refreshLiveWindowGeometry() {
        try (XLock lock = xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.DRAWABLE_MANAGER)) {
            refreshLiveWindowGeometryRecursive(
                    xServer.windowManager.rootWindow,
                    xServer.windowManager.rootWindow.getX(),
                    xServer.windowManager.rootWindow.getY());
        }
    ...
    private void refreshLiveWindowGeometryRecursive(Window window, int x, int y) {
        if (!window.attributes.isMapped()) return;
        if (window != xServer.windowManager.rootWindow) {
            Drawable content = window.getContent();
            if (content != null) {
                ...
                for (RenderableWindow rw : renderableWindows) {
                    if (rw.content == content) {
                        rw.rootX = (short) rootX;
                        rw.rootY = (short) rootY;
                        break;
                    }
                }
            }
        ...
        for (Window child : window.getChildren()) {
            refreshLiveWindowGeometryRecursive(child, child.getX() + x, child.getY() + y);
        }
    }
