...
public class VulkanRenderer
    ...
    private void buildAndSubmitFrame(long handle) {
        ...
        try (XLock lock = xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.DRAWABLE_MANAGER)) {
            ...
            // Prefer the window the user is directly interacting with (input focus, i.e. the
            ...
            for (int i = startIndex; i < renderableWindows.size() && winCount < MAX_WINDOWS; i++) {
                ...
                // A window that owns a mapped child is a container/frame (e.g. the Win32
                // title-bar/border chrome that Wine draws around the real client area) — the
                // real game render always lives in a leaf descendant instead, so containers
                // are ranked strictly below every leaf window. They are NOT dropped to zero:
                // native find_game_window() treats source_width/height as its single strongest
                // signal (+180 of its score), and if we ever hand it 0 here (e.g. no interacted
                // window matched any leaf this frame), that signal disappears and the remaining
                // aspect/scale scores tie between frame and child — ties there resolve to
                // whichever window comes first in array order, which is always the parent frame
                // (collectRenderableWindows visits parent before children). So containers must
                // stay eligible as a last-resort fallback rather than being excluded outright.
                boolean isContainerFrame = rw.window != null && hasMappedChild(rw.window);
                int candidateTier;
                if (isContainerFrame) {
                    candidateTier = 0;
                } else if (interactedWindow != null && rw.window != null
                        && isDescendantOf(rw.window, interactedWindow)) {
                    // The actual game render surface is normally a child window of the
                    // focused top-level window (which itself is just the title-bar/frame
                    // decoration), so descendants take priority over the frame itself.
                    candidateTier = 3;
                } else if (interactedWindow != null && rw.window == interactedWindow) {
                    candidateTier = 2;
                } else {
                    candidateTier = 1;
                }
                if (candidateW > 0 && candidateH > 0) {
                    if (candidateTier > sourceTier
                            || (candidateTier == sourceTier && candidateArea > sourceArea)) {
                        sourceW = candidateW;
                        sourceH = candidateH;
                        sourceArea = candidateArea;
                        sourceTier = candidateTier;
                        int[] sourceRoot = effectiveRootPositions.get(rw.window);
                        sourceX = sourceRoot != null ? sourceRoot[0] : rw.rootX;
                        sourceY = sourceRoot != null ? sourceRoot[1] : rw.rootY;
                        // The touch rectangle follows the actual client drawable, not the
                        // frame-generation source dimensions. This is important when the
                        // presented source is a cropped/scanout surface whose size differs
                        // from the X11 window geometry.
                        sourceRectW = Math.max(1, Short.toUnsignedInt(drawable.width));
                        sourceRectH = Math.max(1, Short.toUnsignedInt(drawable.height));
                    }
                }
        ...
        // Publish Game Rect before writing the presentation state.  The viewport/scissor
        // below is deliberately computed from the same source rectangle, so there is no
        // one-frame lag between the input boundary and the GPU presentation boundary.
        if (sourceW > 0 && sourceH > 0 && sourceRectW > 0 && sourceRectH > 0) {
            viewTransformation.setGameRectFromScene(
                    sourceX, sourceY, sourceRectW, sourceRectH, surfaceWidth, surfaceHeight);
        } else {
            viewTransformation.clearGameRect();
        }

        // IMPORTANT: viewport remains the complete display transform. The Game Rect is a
        // CLIP region, not a replacement viewport. Replacing the viewport with the child
        // window rectangle would rescale the entire Wine desktop into the game window and
        // is exactly what made the previous patch wrong.
        final int viewportX = viewTransformation.viewOffsetX;
        final int viewportY = viewTransformation.viewOffsetY;
        final int viewportW = viewTransformation.viewWidth;
        final int viewportH = viewTransformation.viewHeight;
        buf.putInt(OFF_VIEWPORT,      viewportX);
        buf.putInt(OFF_VIEWPORT + 4,  viewportY);
        buf.putInt(OFF_VIEWPORT + 8,  viewportW);
        buf.putInt(OFF_VIEWPORT + 12, viewportH);

        // Hard GPU-side clip for the compositor scene. This prevents scene pixels from
        // leaking outside the selected client/Game Rect while preserving the original
        // root->surface transform. If no game window is known yet, clip to the normal
        // rendered view so startup remains safe.
        final boolean hasGameRect = viewTransformation.gameRectValid
                && viewTransformation.gameRectWidth > 0
                && viewTransformation.gameRectHeight > 0;
        final int clipX = hasGameRect ? viewTransformation.gameRectX : viewportX;
        final int clipY = hasGameRect ? viewTransformation.gameRectY : viewportY;
        final int clipW = hasGameRect ? viewTransformation.gameRectWidth : viewportW;
        final int clipH = hasGameRect ? viewTransformation.gameRectHeight : viewportH;
        int sX = Math.max(0, clipX);
        int sY = Math.max(0, clipY);
        int sRight = Math.min(surfaceWidth, clipX + Math.max(0, clipW));
        int sBottom = Math.min(surfaceHeight, clipY + Math.max(0, clipH));
    ...
    public void onUpdateWindowGeometry(final Window window, boolean resized) {
        // A move of a parent/frame also moves every descendant in root coordinates.
        // Updating only the moved Window's cached rootX/rootY leaves a reparented
        // game client at its old screen position while Wine's title-bar/frame moves
        // to the new position (the visible "frame detached from game" bug).
        //
        // Rebuild the renderable-window cache for both moves and resizes so all
        // descendant client surfaces receive the exact same root-coordinate update.
        // updateScene() is already coalesced by the render request path, so this is
        // safe during continuous title-bar dragging.
        xServerView.queueEvent(this::updateScene);
        requestRenderCoalesced();
    }
    ...
    private void collectRenderableWindows(Window window, int x, int y) {
        ...
        if (window != xServer.windowManager.rootWindow) {
            ...
            if (viewable) renderableWindows.add(new RenderableWindow(window, window.getContent(), x, y));
        }
    ...
    // Walks up the parent chain to determine whether `window` lives inside `ancestor`'s
    // subtree (e.g. a game's GL/Vulkan render surface nested under its top-level frame).
    private static boolean isDescendantOf(Window window, Window ancestor) {
    ...
    // True when `window` has at least one mapped child. Wine reparents the actual game
    // content into a child window and only draws the title-bar/border chrome on the
    // top-level window itself, so any window with a mapped child is chrome, never content.
    private static boolean hasMappedChild(Window window) {
        for (Window child : window.getChildren()) {
            if (child.attributes.isMapped()) return true;
        }
        return false;
    }
