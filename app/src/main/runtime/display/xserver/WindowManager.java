...
public class WindowManager extends XResourceManager {
  ...
  // Minimum amount (px) of a top-level window that must stay reachable on screen when the
  // guest drags/moves it. Keeps the title bar grabbable instead of letting the whole window
  // disappear past an edge.
  private static final int MIN_VISIBLE_MARGIN_PX = 48;

  /**
   * Bounds policy for top-level application windows during user-driven moves/resizes
   * (title-bar drag, edge-resize, restore from maximize, etc.).
   *
   * Unlike {@link #centeredTopLevelPosition}, this does not force a fixed position - it only
   * clamps the requested x/y so the window can't be dragged fully off the real screen, while
   * still letting the guest (Wine's window manager) freely place/move/resize the window
   * anywhere within those bounds. Windows that cover the whole screen (fullscreen/borderless
   * games) are passed through untouched so their client area is never clipped or shifted.
   */
  private short[] clampTopLevelPosition(short x, short y, short width, short height) {
    int screenWidth = rootWindow.getWidth();
    int screenHeight = rootWindow.getHeight();

    boolean coversScreen =
        x <= 0 && y <= 0 && x + width >= screenWidth && y + height >= screenHeight;
    if (coversScreen) return new short[] {x, y};

    int marginX = Math.min(MIN_VISIBLE_MARGIN_PX, Math.max(width, 1));
    int marginY = Math.min(MIN_VISIBLE_MARGIN_PX, Math.max(height, 1));

    int minX = -(width - marginX);
    int maxX = Math.max(minX, screenWidth - marginX);
    // Never let the title bar be dragged above the visible top edge - it must stay reachable.
    int minY = 0;
    int maxY = Math.max(minY, screenHeight - marginY);

    int clampedX = Math.min(Math.max((int) x, minX), maxX);
    int clampedY = Math.min(Math.max((int) y, minY), maxY);

    return new short[] {(short) clampedX, (short) clampedY};
  }
  ...
  private void changeWindowGeometry(Window window, short x, short y, short width, short height) {
    if (window.isInputOutput() && isTopLevel(window.getParent())) {
      // Clamp (not force-center) here: this path handles guest-driven moves/resizes, e.g.
      // dragging the title bar or an edge-resize, and forcing a centered position would
      // fight the drag and snap the window back every time it's configured. Fullscreen/
      // borderless windows are detected and passed through untouched inside the clamp.
      short[] clamped = clampTopLevelPosition(x, y, width, height);
      x = clamped[0];
      y = clamped[1];
    }
