...
class TouchpadView(
    ...
    private fun updateXform(outerWidth: Int, outerHeight: Int, innerWidth: Int, innerHeight: Int) {
        ...
        // One authoritative transform for both display and input. Game Rect is an X11/root
        // window rectangle expressed in the same scene coordinates as the renderer.
        vt.forceStretch = renderer.isFullscreen
        vt.update(surfaceW, surfaceH, sceneW, sceneH)

        if (vt.gameRectValid && vt.gameRectWidth > 0 && vt.gameRectHeight > 0) {
            if (vt.effectiveMode == ViewTransformation.FILL_MODE_STRETCH) {
                val sx = vt.gameSceneWidth.toFloat() / vt.gameRectWidth.toFloat()
                val sy = vt.gameSceneHeight.toFloat() / vt.gameRectHeight.toFloat()
                XForm.set(
                    xform, sx, 0f, 0f, sy,
                    vt.gameSceneX - vt.gameRectX * sx,
                    vt.gameSceneY - vt.gameRectY * sy,
                )
            } else {
                val inv = if (vt.aspect > 0f) 1f / vt.aspect else 1f
                XForm.set(
                    xform, inv, 0f, 0f, inv,
                    vt.gameSceneX - vt.gameRectX * inv,
                    vt.gameSceneY - vt.gameRectY * inv,
                )
            }
            return
        }

        // Before the game window is discovered, keep the legacy whole-surface mapping.
        if (vt.effectiveMode == ViewTransformation.FILL_MODE_STRETCH) {
            XForm.set(xform, vt.sceneScaleX, 0f, 0f, vt.sceneScaleY, 0f, 0f)
        } else {
            val inv = if (vt.aspect > 0f) 1f / vt.aspect else 1f
            XForm.set(xform, inv, 0f, 0f, inv,
                -vt.viewOffsetX * inv, -vt.viewOffsetY * inv)
        }
    }
    ...
    private fun handleTouchDown(event: MotionEvent) {
        val renderer = xServer.renderer
        val vt = renderer?.viewTransformation
        if (simTouchScreen && vt != null && vt.aspect > 0f &&
            !vt.isInsideGameFrame(event.x, event.y)) {
            // The resized frame has no padding. A touch outside it must not be converted
            // into a fake guest coordinate.
            return
        }
