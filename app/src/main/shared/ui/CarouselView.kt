...
fun <T> CarouselView(
    ...
    BoxWithConstraints(
        ...
        // The play/download/folder/add corner buttons are drawn in their own overlay Box
        // by the screen that hosts this carousel (aligned to BottomCenter over the whole
        // screen) - they aren't part of this composable's layout, so this carousel has no
        // way to know about them unless we explicitly reserve room. Without this, a card
        // sized off the full viewport height can grow tall enough to sit underneath them.
        val cornerActionsSafeSpace = 84.dp
        // Item boxes below add 28.dp of extra height on top of baseCardHeight so the
        // 1.12x hover/focus scale has room to grow without clipping - reserve that here too.
        val itemScaleBuffer = 28.dp
        val verticalReserved = 24.dp + 24.dp + navBottomInset + cornerActionsSafeSpace + itemScaleBuffer
        val availableCardHeight = (maxHeight - verticalReserved).coerceAtLeast(120.dp)
        val cardAspectRatio = 1.2f // height = width * cardAspectRatio

        // Landscape/tablet: the viewport is short and wide. Cards used to be sized purely
        // off width, which capped their height well below what the screen actually had
        // room for and left the cover art looking squeezed/short. Size off the real
        // available height instead so the cover uses the vertical space it's actually
        // given, then derive width from that (capped so at least ~1.6 cards' worth of
        // width still shows, preserving the carousel's peek-of-next-card feel).
        ...
        val baseCardWidth: Dp
        val baseCardHeight: Dp
        if (isPortrait) {
            baseCardWidth = ((maxHeight * 0.56f) / cardAspectRatio).coerceAtMost(maxWidth * 0.62f)
            baseCardHeight = (baseCardWidth * cardAspectRatio).coerceAtMost(availableCardHeight)
        } else {
            baseCardHeight = availableCardHeight
            baseCardWidth = (baseCardHeight / cardAspectRatio).coerceAtMost(maxWidth * 0.34f)
        }
